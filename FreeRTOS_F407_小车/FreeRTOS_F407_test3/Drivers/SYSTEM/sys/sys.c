 ...
 *-----------------------------------------------------------------------------
 * File name   : sys.c
 * Description : 系统初始化代码(包括时钟配置/中断管理/GPIO设置等)
 * Author      : nbufe
 * Date        : 2023-09-14
 *-----------------------------------------------------------------------------
 * 实验平台: C32F407Pro 开发板
 * 修改说明: V1.0 20230914(第一次发布)
 *
**/
...
/*
 * 功能: 执行: WFI指令(执行完该指令进入低功耗状态, 等待中断唤醒)
 * 参数: None
 * 返回: None
 * 说明: None
 */
...
/*
 * 功能: 时钟设置函数
 * 参数: plln: PLL1倍频系数(PLL倍频), 取值范围: 64~432.
 * 			 pllm: PLL1预分频系数(进PLL之前的分频), 取值范围: 2~63.
 *       pllp: PLL1的p分频系数(PLL之后的分频), 分频后作为系统时钟, 取值范围: 2,4,6,8.(仅限这4个值!)
 *       pllq: PLL1的q分频系数(PLL之后的分频), 取值范围: 2~15.
 * 返回: 错误代码: 0, 成功; 1, 错误;
 * 说明: Fvco: VCO频率
